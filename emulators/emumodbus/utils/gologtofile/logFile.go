...
func InitCtsLogs(inFilePath string, inFileName string, delLogFiles bool, inLogLevel int) (int, error) {
	var err error = nil
	var erctr int = 0
	...
	for f := range allFiles {
		...
		if strings.Contains(fileName, ".log") {
			if delLogFiles {
				os.Remove(filePath)
				fmt.Printf("InitCtsLogs: filePath[%s] has been deleted\n", filePath)
			} else {
				fmt.Printf("InitCtsLogs: filePath[%s] Not deleted\n", filePath)
			}
		}
	}
	if len(inFileName) == 0 {
