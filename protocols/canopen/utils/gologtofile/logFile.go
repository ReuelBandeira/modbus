...
func LogFileSizeMonGoRoutine(logFileHandle *os.File, inFilePath string, inFileName string, maxLogFileSize int, inLogLevel int) {
	...
	for {
		...
		// Check if logFileName file exists
		_, erroFile := os.Stat(strFullInLogFileName)
		if os.IsNotExist(erroFile) {
			// LogFileName do not exists because date was changed
			// BEGIN: Fix bug 26/11/2023
			CtsLog.Warn("Log File Name[%s] Not Exists date changed!! Start log on new date!!!\n", strFullInLogFileName)
			CtsLog.SetOutput(io.MultiWriter(os.Stdout))

			//Close current LogFile
			err := CtsLogFileHandle.Close()
			if err != nil {
				CtsLog.Error("Close Feil \r\n strFullbkupLogFileName[%s]err[%s]\n\n", CtsLogFileHandle, err)
				break
			}

			// Open New log file name
			CtsLogFileHandle, err = openLogFile(strFullInLogFileName)
			if err != nil {
				CtsLog.Error("openLogFile Fail CtsLogFileHandle[%v]\r\n err[%s]\n\n", CtsLogFileHandle, err)
			} else {
				// Setup LogFile to logger
				CtsLog.SetOutput(io.MultiWriter(os.Stdout, CtsLogFileHandle))
				CtsLog.Debug("InitCtsLogs:Parameters\n Running               OS[%s:%s] %d bits Host[%s]\n              LogFilePath[%s]\n              LogFileName[%s]\n                 LogLevel[%d]\n\n",
					runtime.GOOS,
					runtime.GOARCH,
					32<<(^uint(0)>>63),
					OsBits,
		...
		if err != nil {
			...
			// File do not exists because date was changed
			// BEGIN: Fix bug 13/11/2023
			CtsLog.Warn("Log File Name[%s] Not Exists date changed!! Start log on new date!!!\n", strFullInLogFileName)
			CtsLog.SetOutput(io.MultiWriter(os.Stdout))
