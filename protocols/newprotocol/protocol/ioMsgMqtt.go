...
func MQTTSendMessageAutomatic(msg utilsPkg.NewDeviceExitPayloadJsonMsg) error {
	...
	if !utilsPkg.MqttClient.IsConnected() {
		logPkg.CtsLog.Error("ioMsgMqtt:MQTTSendMessageAutomatic: MQTT Client not connected to MQTT Brocker")
		return fmt.Errorf("MQTT Client not connected to MQTT Brocker")
	} else {
...
func MQTTSendProtocolStatus(status string) {
	...
	data.Data.Name = "newprotocol" ///TODO change to your protocol name
	jsonPayload, err := json.Marshal(data)
	if err != nil {
...
func MQTTSendGeneralDeviceStatus(device utilsPkg.DevSettings, status string) {
...
func StartReadFile() {
	...
	// Create a map to keep track of the originalData devices by ID
